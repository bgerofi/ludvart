...
def system_prompt(tools: Sequence[ToolSpec]) -> str:
    ...
    return (
        ...
        "\" quotes, and '->' for arrows. The only exception is text that "
        "cannot be written in ASCII at all, such as Japanese: write that in "
        "UTF-8 when the terminal supports it. The exception covers the words "
        "themselves, not the punctuation and decoration around them, which "
        "stay ASCII.\n\n"
        + LUDVART_HELPERS_DOC
        + load_self_md()
    )
